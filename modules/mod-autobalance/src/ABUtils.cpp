...
namespace
{
uint32 GetLfgLookupMapId(uint32 mapId)
{
    switch (mapId)
    {
        case 1000: return 109; // Temple of Atal'Hakkar
        case 1001: return 34;  // The Stockade
        case 1002: return 70;  // Uldaman
        case 1003: return 47;  // Razorfen Downs
        case 1004: return 129; // Razorfen Kraul
        case 1005: return 349; // Maraudon
        case 1006: return 289; // Scholomance
        case 1007: return 36;  // The Deadmines
        case 1008: return 33;  // Shadowfang Keep
        case 1009: return 90;  // Gnomeregan
        case 1010: return 229; // Blackrock Spire
        case 1011: return 429; // Dire Maul
        case 1012: return 209; // Zul'Farrak
        case 1013: return 48;  // Blackfathom Deeps
        case 1014: return 329; // Stratholme
        case 1015: return 230; // Blackrock Depths
        case 1016: return 389; // Ragefire Chasm
        case 1017: return 43;  // Wailing Caverns
        case 1018: return 189; // Scarlet Monastery
...
//
std::map<uint32, AutoBalanceLevelScalingDynamicLevelSettings> LoadDynamicLevelOverrides(std::string dungeonIdString)
{
    std::string       delimitedValue;
    std::stringstream dungeonIdStream;

    std::map<uint32, AutoBalanceLevelScalingDynamicLevelSettings> overrideMap;
...
AutoBalanceMapInfo* GetMapInfo(Map* map)
{
    ...
    uint32 const lfgMapId = GetLfgLookupMapId(map->GetId());

    if (lfgMapId != map->GetId())
    {
        LOG_DEBUG("module.AutoBalance", "AutoBalance::InitializeMap: Map {} ({}{}) | Using source map {} for LFG level ranges.",
            map->GetMapName(),
            map->GetId(),
            map->GetInstanceId() ? "-" + std::to_string(map->GetInstanceId()) : "",
            lfgMapId
    ...
    LFGDungeonEntry const* dungeon = GetLFGDungeon(lfgMapId, map->GetDifficulty());
    if (dungeon) {
    ...
    else if (map->IsHeroic())
    {
        ...
        if (map->GetDifficulty() == DUNGEON_DIFFICULTY_HEROIC)
            nonHeroicDungeon = GetLFGDungeon(lfgMapId, DUNGEON_DIFFICULTY_NORMAL);
        else if (map->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC)
            nonHeroicDungeon = GetLFGDungeon(lfgMapId, RAID_DIFFICULTY_10MAN_NORMAL);
        else if (map->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
            nonHeroicDungeon = GetLFGDungeon(lfgMapId, RAID_DIFFICULTY_25MAN_NORMAL);
