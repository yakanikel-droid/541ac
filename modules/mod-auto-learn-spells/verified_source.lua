--[[==============================================================
    Auto Learn Spells (Автоматическое изучение заклинаний)
    Год создания: 2026

    Назначение:
    Автоматически изучает доступные заклинания при повышении уровня
    персонажа. Поддерживается получение сразу нескольких уровней.
================================================================]]

print(">> auto_learn_spell.lua успешно загружен.")

----------------------------------------------------
-- Настройки
----------------------------------------------------

-- Проверять и восстанавливать отсутствующие заклинания
-- при входе персонажа в игру.
local LOGIN_CHECK = true

----------------------------------------------------
-- Идентификаторы классов
----------------------------------------------------

local CLASS_WARRIOR      = 1   -- Воин
local CLASS_PALADIN      = 2   -- Паладин
local CLASS_HUNTER       = 3   -- Охотник
local CLASS_ROGUE        = 4   -- Разбойник
local CLASS_PRIEST       = 5   -- Жрец
local CLASS_DEATH_KNIGHT = 6   -- Рыцарь смерти
local CLASS_SHAMAN       = 7   -- Шаман
local CLASS_MAGE         = 8   -- Маг
local CLASS_WARLOCK      = 9   -- Чернокнижник
local CLASS_DRUID        = 11  -- Друид

--[[==============================================================
    Классовые и квестовые способности
    (не входят в обычный список автоматически изучаемых заклинаний)
    ...
    31700 — Полёты в Нордсколе (уровень 40)
    44140 — Полёты по всему Азероту
    33388 — Начальный навык верховой езды
================================================================]]

local SKILL = { -- Только обычные классовые заклинания; проверено по Spell.dbc, SkillLineAbility.dbc и spell_ranks.sql
	[CLASS_WARRIOR] = { -- Воин
		[1] = {78, 6673},
		[4] = {100, 772},
		[6] = {6343, 34428},
	...
	[CLASS_DRUID] = { -- Друид
		...
		[6] = {467, 5177},
		[8] = {339, 5186},
		[10] = {99, 1058, 5232, 5487, 6807, 8924, 16689},
		[12] = {5229, 8936},
...
-- Изучает заклинания в диапазоне уровней от fromLevel до toLevel
local function LearnSpell(player, fromLevel, toLevel)
	-- Если начальный уровень больше конечного, ничего не делаем
	...
	-- Получаем таблицу заклинаний для данного класса
	local classSpells = SKILL[playerClass]

	-- Если для класса нет списка заклинаний
	if not classSpells then
		print("AutoLearnSpell: отсутствует таблица заклинаний для класса:", playerClass)
		return
	end

	-- Проходим по всем уровням в указанном диапазоне
	for level = fromLevel, toLevel do
		local levelSpells = classSpells[level]

		-- Если для уровня есть список заклинаний
		if levelSpells then
			for _, spellId in pairs(levelSpells) do
				-- Изучаем только неизвестные заклинания
				if not player:HasSpell(spellId) then
					player:LearnSpell(spellId)

					-- Отладка:
					-- print("AutoLearnSpell:", player:GetName(), spellId)
				end
			end
		end
	end
...
----------------------------------------------------
-- Автоматическое изучение при повышении уровня
----------------------------------------------------

local function OnLevelChange(_, player, oldLevel)
	-- Проверяем всю цепочку заклинаний с 1 уровня,
	-- чтобы не пропустить предыдущие ранги.
	LearnSpell(player, 1, player:GetLevel())
end

----------------------------------------------------
-- Проверка заклинаний при входе в игру
----------------------------------------------------

local function OnLogin(_, player)
	-- Если проверка отключена в настройках
	if not LOGIN_CHECK then
		return
	end

	-- Восстанавливаем все отсутствующие заклинания
	-- до текущего уровня персонажа.
	LearnSpell(player, 1, player:GetLevel())
end

----------------------------------------------------
-- Регистрация событий Eluna
----------------------------------------------------
