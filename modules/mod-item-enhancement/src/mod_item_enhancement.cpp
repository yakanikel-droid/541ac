...
struct Settings
{
    ...
    std::map<uint8, float> BonusesItemLevel100;
    std::map<uint8, float> BonusesItemLevel200;
    std::map<uint8, uint32> TransferMaterialCounts;
...
std::map<uint8, uint32> ParseLevelCounts(std::string const& value)
{
    ...
    for (std::string const& row : Split(value, ','))
    {
        std::vector<std::string> fields = Split(row, ':');
        if (fields.size() == 2)
            result[uint8(Number<uint32>(fields[0]))] = Number<uint32>(fields[1]);
    }
    return result;
}
...
void LoadConfig()
{
    ...
    Config.BonusesItemLevel100 =
        ParseLevelFloats(sConfigMgr->GetOption<std::string>("ItemEnhancement.LevelBonusesItemLevel100", ""));
    Config.BonusesItemLevel200 =
        ParseLevelFloats(sConfigMgr->GetOption<std::string>("ItemEnhancement.LevelBonusesItemLevel200", ""));
    Config.TransferMaterialCounts = ParseLevelCounts(
        sConfigMgr->GetOption<std::string>("ItemEnhancement.Transfer.MaterialCounts", ""));
    ...
    for (auto& [level, bonus] : Config.BonusesItemLevel100)
        bonus = std::isfinite(bonus) ? std::max(bonus, 0.0f) : 0.0f;
    for (auto& [level, bonus] : Config.BonusesItemLevel200)
        bonus = std::isfinite(bonus) ? std::max(bonus, 0.0f) : 0.0f;
...
float GetBonus(ItemTemplate const* proto, uint8 level)
{
    std::map<uint8, float> const* bonuses = &Config.Bonuses;
    if (proto && proto->ItemLevel <= 100 && !Config.BonusesItemLevel100.empty())
        bonuses = &Config.BonusesItemLevel100;
    else if (proto && proto->ItemLevel <= 200 && !Config.BonusesItemLevel200.empty())
        bonuses = &Config.BonusesItemLevel200;

    auto itr = bonuses->find(level);
    return itr == bonuses->end() ? 0.0f : itr->second;
}

uint32 GetTransferMaterialCount(uint8 level)
{
    auto itr = Config.TransferMaterialCounts.find(level);
    if (itr == Config.TransferMaterialCounts.end())
        return Config.TransferMaterialCount;
    return uint32(itr->second);
}
...
bool IsEligible(Item const* item, std::string& error)
{
    ...
    {
        error = "Предметы со случайными характеристиками "
            "нельзя улучшать.";
        return false;
    }
...
std::string BuildStatPreview(ItemTemplate const* proto, uint8 level)
{
    ...
    float bonus = GetBonus(proto, level);
    bool first = true;
...
std::string BuildDamagePreview(ItemTemplate const* proto, uint8 level)
{
    ...
    float multiplier = 1.0f + GetBonus(proto, level) / 100.0f;
    std::ostringstream result;
...
void SendInspect(Player* player, char const* kind, uint8 first, uint8 second, Item* item)
{
    ...
    Send(player, Acore::StringFormat("INSPECT\t{}\t{}\t{}\t{}\t{:.2f}\t{}\t{}", kind, first, second,
        level, GetBonus(item->GetTemplate(), level), BuildStatPreview(item->GetTemplate(), level),
...
void SendPreview(Player* player)
{
    ...
    Send(player, Acore::StringFormat(
        "PREVIEW\t{}\t{}\t{}\t{}\t{:.2f}\t{}\t{:.2f}\t{:.2f}\t{}\t{:.2f}\t{}\t{}\t{}\t{}",
        item->GetEntry(), current, target, item->GetTemplate()->ItemLevel, chance, cost,
        GetBonus(item->GetTemplate(), current), GetBonus(item->GetTemplate(), target), materialType, materialValue,
        BuildStatPreview(item->GetTemplate(), current),
...
bool Attempt(Player* player)
{
    ...
    if (success)
    {
        ...
        Send(player, Acore::StringFormat("RESULT\tSUCCESS\t{}\t{}\t{}",
            target, GetBonus(item->GetTemplate(), target), consumeMaterial ? 1 : 0));
    }
    else
    {
...
void SendTransferPreview(Player* player)
{
    ...
    if (source && source->GetTemplate()->ItemLevel < 201)
    {
        Send(player, "ERROR\tИсходный предмет должен иметь ItemLevel не ниже 201.");
        return;
    }

    if (target && target->GetTemplate()->ItemLevel < 201)
    {
        Send(player, "ERROR\tЦелевой предмет должен иметь ItemLevel не ниже 201.");
        return;
    }

    uint8 sourceLevel = source ? GetLevel(source) : 0;
    uint32 materialCount = sourceLevel ? GetTransferMaterialCount(sourceLevel) : 0;
    Send(player, Acore::StringFormat("TRANSFER_PREVIEW\t{}\t{}\t{}\t{}\t{}\t{:.2f}\t{}\t{}",
        source ? source->GetEntry() : 0, sourceLevel, target ? target->GetEntry() : 0,
        sourceLevel ? Config.TransferMaterialId : 0, materialCount,
        source ? GetBonus(source->GetTemplate(), sourceLevel) : 0.0f,
...
bool Transfer(Player* player)
{
    ...
    if (source->GetTemplate()->ItemLevel < 201 || target->GetTemplate()->ItemLevel < 201)
    {
        Send(player,
            "ERROR\tДля переноса оба предмета должны иметь "
            "ItemLevel не ниже 201.");
        return true;
    }
    if (source == target)
    {
    ...
    uint32 materialCount = GetTransferMaterialCount(level);
    if (!Config.TransferMaterialId || !materialCount ||
        !player->HasItemCount(Config.TransferMaterialId, materialCount))
    {
    ...
    player->DestroyItemCount(Config.TransferMaterialId, materialCount, true);
...
public:
        ...
        uint32 statType, int32& value) override
    {
        ...
        if (!level)
            return;
        value += int32(std::lround(value * ItemEnhancement::GetBonus(item->GetTemplate(), level) / 100.0f));
    }
        ...
        float& maxDamage, uint8 /*damageIndex*/) override
    {
        ...
        if (!level)
            return;
        float multiplier = 1.0f + ItemEnhancement::GetBonus(item->GetTemplate(), level) / 100.0f;
...
public:
    ...
    static bool HandleInspectChat(ChatHandler* handler, uint32 entry, uint8 level)
    {
        ...
        ItemEnhancement::Send(player, Acore::StringFormat("CHAT_INSPECT\t{}\t{}\t{:.2f}\t{}\t{}", entry,
            level, ItemEnhancement::GetBonus(itemTemplate, level),
            ItemEnhancement::BuildStatPreview(itemTemplate, level),
...
public:
    ...
    void OnSpellPrepare(Spell* spell, Unit* /*caster*/, SpellInfo const* spellInfo) override
    {
        ...
        if (!level)
            return;
        float multiplier = 1.0f + ItemEnhancement::GetBonus(castItem->GetTemplate(), level) / 100.0f;
