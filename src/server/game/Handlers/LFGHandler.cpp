...
void WorldSession::SendLfgUpdateProposal(lfg::LfgProposal const& proposal)
{
    ...
    for (auto const& pguid : ordered)
    {
        ...
        // The 3.3.5a client accepts exactly one combat role in a proposal
        // member entry. PLAYER_ROLE_LEADER is carried separately by the
        // proposal leader flag; sending LEADER, NONE or a combined mask makes
        // GetLFGProposalMember() return "UNKNOWN" and breaks LFDFrame.lua.
        uint8 proposalRole = player.role &
            (lfg::PLAYER_ROLE_TANK | lfg::PLAYER_ROLE_HEALER | lfg::PLAYER_ROLE_DAMAGE);

        if (proposalRole != lfg::PLAYER_ROLE_TANK &&
            proposalRole != lfg::PLAYER_ROLE_HEALER &&
            proposalRole != lfg::PLAYER_ROLE_DAMAGE)
        {
            uint8 const storedRole = sLFGMgr->GetRoles(pguid) &
                (lfg::PLAYER_ROLE_TANK | lfg::PLAYER_ROLE_HEALER | lfg::PLAYER_ROLE_DAMAGE);

            if (storedRole & lfg::PLAYER_ROLE_TANK)
                proposalRole = lfg::PLAYER_ROLE_TANK;
            else if (storedRole & lfg::PLAYER_ROLE_HEALER)
                proposalRole = lfg::PLAYER_ROLE_HEALER;
            else
                proposalRole = lfg::PLAYER_ROLE_DAMAGE;

            LOG_ERROR("lfg", "Invalid proposal role {} for player [{}]; sending normalized role {}",
                player.role, pguid.ToString(), proposalRole);
        }

        data << uint32(proposalRole);                      // Exact combat role
