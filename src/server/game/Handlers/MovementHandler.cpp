...
namespace
{
struct HeroicClassicTriggerState
{
    uint32 mapId = 0;
    bool initialized = false;
    std::unordered_set<uint32> blockedTriggers;
};

std::unordered_map<uint32, HeroicClassicTriggerState> HeroicClassicTriggerStates;

bool HandleHeroicClassicAreaTrigger(Player* player)
{
    uint32 mapId = player->GetMapId();
    uint32 playerGuid = player->GetGUID().GetCounter();

    if (mapId < 1000 || mapId > 1018)
    {
        HeroicClassicTriggerStates.erase(playerGuid);
        return false;
    }
    ...
    HeroicClassicTriggerState& state = HeroicClassicTriggerStates[playerGuid];
    if (state.mapId != mapId)
    {
        state.mapId = mapId;
        state.initialized = false;
        state.blockedTriggers.clear();
    }

    // On the first movement packet after entering a heroic copy, remember all
    // exit triggers at or close to the original arrival point. They stay
    // blocked until the player has moved completely clear of their volume.
    if (!state.initialized)
    {
        for (uint32 triggerId = 5873; triggerId <= 5965; ++triggerId)
        {
            AreaTrigger const* trigger = sObjectMgr->GetAreaTrigger(triggerId);
            if (trigger && trigger->map == mapId && player->IsInAreaTriggerRadius(trigger, 5.0f))
                state.blockedTriggers.insert(triggerId);
        }
    ...
    {
        AreaTrigger const* trigger = sObjectMgr->GetAreaTrigger(triggerId);
        if (!trigger || trigger->map != mapId)
            continue;

        if (state.blockedTriggers.find(triggerId) != state.blockedTriggers.end())
        {
            if (!player->IsInAreaTriggerRadius(trigger, 5.0f))
                state.blockedTriggers.erase(triggerId);

            continue;
        }

        if (!player->IsInAreaTriggerRadius(trigger))
            continue;
        ...
        player->TeleportTo(teleport->target_mapId, teleport->target_X, teleport->target_Y,
            teleport->target_Z, teleport->target_Orientation, TELE_TO_NOT_LEAVE_TRANSPORT);
        return true;
    }
...
bool WorldSession::ProcessMovementInfo(MovementInfo& movementInfo, Unit* mover, Player* plrMover, WorldPacket& recvData)
{
    ...
    // Custom classic heroic maps use copied geometry. The 3.3.5 client does not
    // send CMSG_AREATRIGGER for copied map IDs, so check their exit volumes here.
    if (plrMover && HandleHeroicClassicAreaTrigger(plrMover))
        return false;
