...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
    ...
    uint32 targetMapId = at->target_mapId;
    float targetX = at->target_X;
    float targetY = at->target_Y;
    float targetZ = at->target_Z;
    float targetOrientation = at->target_Orientation;

    if (player->GetDungeonDifficulty() == DUNGEON_DIFFICULTY_HEROIC)
        targetMapId = GetHeroicClassicCloneMapId(targetMapId);

    bool teleported = false;
    if (player->GetMapId() != targetMapId)
    {
       if (Map::EnterState denyReason = sMapMgr->PlayerCannotEnter(targetMapId, player, false))
        {
            ...
            if (reviveAtTrigger) // check if the player is touching the areatrigger leading to the map his corpse is on
            {
                ...
                {
                    if (player->GetCorpseLocation().GetMapId() == targetMapId)
                    {
    ...
    if (!teleported)
        player->TeleportTo(targetMapId, targetX, targetY, targetZ, targetOrientation,
...
void WorldSession::HandleSetDungeonDifficultyOpcode(WorldPackets::Instance::SetDungeonDifficultyClient& packet)
{
    ...
    // Queue a forced visibility refresh for the next player update. This
    // updates heroic entrance skulls without waiting for player movement and
    // without delaying the difficulty-selection UI.
    if (_player->IsInWorld())
        _player->UpdateObjectVisibility(true);
}
