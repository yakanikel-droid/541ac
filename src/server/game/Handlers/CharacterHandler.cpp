...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    // Reject races that exist in ChrRaces.dbc but are not enabled by RaceMgr.
    // The playable race mask is uint32 and supports race IDs 1 through 32.
    if (createInfo->Race == 0 || createInfo->Race > 32 ||
        !(sRaceMgr->GetPlayableRaceMask() & (1u << (createInfo->Race - 1))))
    {
        SendCharCreate(CHAR_CREATE_FAILED);
        LOG_ERROR("network.opcode", "Account {} tried to create disabled or non-playable race {}", GetAccountId(), createInfo->Race);
        return;
    }
...
void WorldSession::HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo, PreparedQueryResult result)
{
    ...
    if (oldRace != factionChangeInfo->Race)
    {
        ...
        if (factionChangeInfo->Race != RACE_ORC && factionChangeInfo->Race != RACE_HUMAN)
        {
            ...
            // Custom races use their faction language from playercreateinfo_skills.
            // Do not append a prepared statement with an unset language ID.
            if (raceLanguage)
            {
                stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_SKILL_LANGUAGE);
                stmt->SetData(0, lowGuid);
                stmt->SetData(1, raceLanguage);
                trans->Append(stmt);
            }
        }
