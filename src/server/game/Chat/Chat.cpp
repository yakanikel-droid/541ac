...
{
    constexpr std::string_view AddonPrefix = "Nova\t";
    constexpr std::size_t OpcodePosition = AddonPrefix.size();
    constexpr std::size_t EchoPosition = OpcodePosition + 1;
    constexpr std::size_t CommandPosition = EchoPosition + 4;

    if (str.size() < CommandPosition ||
        str.compare(0, AddonPrefix.size(), AddonPrefix) != 0)
        return false;

    char opcode = str[OpcodePosition];
    if (!opcode)
        return false;

    if (!str[EchoPosition] || !str[EchoPosition + 1] ||
        !str[EchoPosition + 2] || !str[EchoPosition + 3])
        return false;

    echo = str.substr(EchoPosition);
    ...
    switch (opcode)
    {
        ...
        case 'i': // i Issue command
            if (str.size() <= CommandPosition || !str[CommandPosition])
                return false;
            humanReadable = (opcode == 'h');
            if (_ParseCommands(str.substr(CommandPosition)))
            {
                if (!hadAck)
                    SendAck();
...
{
    ASSERT(echo.size());
    std::string ack = "Nova\ta";
    ack.resize(11);
    memcpy(&ack[6], echo.data(), 4);
    ack[10] = '\0';
    Send(ack);
    hadAck = true;
}
...
{
    ASSERT(echo.size());
    std::string ok = "Nova\to";
    ok.resize(11);
    memcpy(&ok[6], echo.data(), 4);
    ok[10] = '\0';
    Send(ok);
}
...
{
    ASSERT(echo.size());
    std::string fail = "Nova\tf";
    fail.resize(11);
    memcpy(&fail[6], echo.data(), 4);
    fail[10] = '\0';
    Send(fail);
}
