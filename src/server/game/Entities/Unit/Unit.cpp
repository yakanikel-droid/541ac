...
void Unit::RestoreDisplayId()
{
	...
	// SetDisplayId() changes UNIT_FIELD_BYTES_0 gender using creature_model_info.
	// For custom player models that information may be missing or incorrect.
	// Restore the player's real gender after leaving a transform.
	if (Player* player = ToPlayer())
	{
		SetByteValue(
			UNIT_FIELD_BYTES_0,
			2,
			player->GetByteValue(PLAYER_BYTES_3, 0)
		);
