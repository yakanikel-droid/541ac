...
bool Player::IsInAreaTriggerRadius(AreaTrigger const* trigger, float delta) const
{
    ...
    if (!trigger || (mapId != trigger->map && GetHeroicClassicSourceMapId(mapId) != trigger->map))
        return false;
...
void Player::SendInitialSpells()
{
    ...
    // Count only cooldowns that will actually be sent to the client.
...
void Player::SetCustomState(uint8 customState)
{
    if (customState > 2)
        customState = 0;
    ...
    {
        TeamId selectedTeam = customState == 1 ? TEAM_ALLIANCE : TEAM_HORDE;
        uint8 factionRace = selectedTeam == TEAM_ALLIANCE ? RACE_HUMAN : RACE_ORC;

        m_team = selectedTeam;
        ChrRacesEntry const* raceEntry = sChrRacesStore.LookupEntry(factionRace);
        SetFaction(raceEntry ? raceEntry->FactionID : 0);
    }
...
void Player::_SaveCharacter(bool create, CharacterDatabaseTransaction trans)
{
    ...
    if (create)
    {
        ...
        stmt->SetData(index++, m_customState);
    }
    else
    {
        // Update query
