...
public:
    ...
    {
        return original && m_customState == 0 ? TeamIdForRace(getRace(true)) : m_team;
    }
    void SetFactionForRace(uint8 race);
    void setTeamId(TeamId teamid) { m_team = teamid; };
    [[nodiscard]] uint8 GetCustomState() const { return m_customState; }
    void SetCustomState(uint8 customState);
    [[nodiscard]] uint32 GetCustomReputationRaceMask() const
    {
        if (m_customState == 1)
            return 1 << (RACE_HUMAN - 1);
        if (m_customState == 2)
            return 1 << (RACE_ORC - 1);
        return 0;
    }
