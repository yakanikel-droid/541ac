...
bool Player::SatisfyQuestRace(Quest const* qInfo, bool msg) const
{
    ...
    uint32 effectiveRaceMask = getRaceMask();
    if (GetCustomState() == 1)
        effectiveRaceMask = sRaceMgr->GetAllianceRaceMask();
    else if (GetCustomState() == 2)
        effectiveRaceMask = sRaceMgr->GetHordeRaceMask();

    if ((reqraces & effectiveRaceMask) == 0)
    {
        if (msg)
