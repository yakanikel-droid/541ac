...
namespace lfg
{
    LFGMgr::LFGMgr(): m_lfgProposalId(1), m_options(sWorld->getIntConfig(CONFIG_LFG_OPTIONSMASK)), m_Testing(sWorld->getBoolConfig(CONFIG_DEBUG_LFG)), m_SoloLfgEnabled(false)
    {
        ...
        else if (task == 2)
        {
            if (lastProposalId != m_lfgProposalId)
            {
                ...
                for (LfgProposalContainer::const_iterator itProposal = ProposalsStore.find(m_lfgProposalId); itProposal != ProposalsStore.end(); ++itProposal)
                {
                    ...
                    // A one-player proposal has nobody else to confirm. The
                    // 3.3.5a client cannot reliably display the ready-status
                    // phase for a one-member proposal and returns UNKNOWN from
                    // GetLFGProposalMember() after AcceptProposal. Complete it
                    // server-side without opening that popup.
                    if (m_SoloLfgEnabled && proposal.players.size() == 1)
                    {
                        ObjectGuid const soloGuid = proposal.players.begin()->first;
                        SetState(soloGuid, LFG_STATE_PROPOSAL);
                        proposal.state = LFG_PROPOSAL_SUCCESS;
                        UpdateProposal(proposalId, soloGuid, true);
                        break; // UpdateProposal erases this proposal.
                    }
