...
{
    return GetBaseReputationForRaceMask(factionEntry, _player->getRaceMask());
}

int32 ReputationMgr::GetBaseReputationForRaceMask(FactionEntry const* factionEntry, uint32 raceMask) const
{
    if (!factionEntry)
        return 0;
...
{
    return GetDefaultStateFlagsForRaceMask(factionEntry, _player->getRaceMask());
}

uint32 ReputationMgr::GetDefaultStateFlagsForRaceMask(FactionEntry const* factionEntry, uint32 raceMask) const
{
    if (!factionEntry)
        return 0;
...
{
    ReinitializeForRaceMask(_player->GetCustomReputationRaceMask());
}

void ReputationMgr::ReinitializeForRaceMask(uint32 raceMask)
{
    _factions.clear();
    ...
    for (unsigned int i = 1; i < sFactionStore.GetNumRows(); i++)
    {
        ...
        if (factionEntry && (factionEntry->reputationListID >= 0))
        {
            ...
            int32 actualBaseReputation = GetBaseReputationForRaceMask(factionEntry, _player->getRaceMask());
            int32 selectedBaseReputation = GetBaseReputationForRaceMask(factionEntry, raceMask);
            newFaction.Standing = selectedBaseReputation - actualBaseReputation;
            newFaction.Flags = GetDefaultStateFlagsForRaceMask(factionEntry, raceMask);
            ...
            UpdateRankCounters(REP_HOSTILE, ReputationToRank(selectedBaseReputation));
...
void ReputationMgr::SetVisible(FactionTemplateEntry const* factionTemplateEntry)
{
    ...
    if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(factionTemplateEntry->faction))
        ...
        if (!(factionEntry->BaseRepRaceMask[1] & _player->GetCustomReputationRaceMask() && factionEntry->BaseRepValue[1] == Reputation_Bottom))
    ...
    if (result)
    {
        do
        {
            ...
            if (factionEntry && (factionEntry->reputationListID >= 0))
            {
                ...
                ReputationRank oldRank = GetRank(factionEntry);

                // update standing to current
                faction->Standing = fields[1].Get<int32>();

                // update counters
                ReputationRank newRank = GetRank(factionEntry);
