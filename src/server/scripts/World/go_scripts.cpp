...
class go_heroic_classic_skull : public GameObjectScript
{
public:
    go_heroic_classic_skull() : GameObjectScript("go_heroic_classic_skull") { }

    struct go_heroic_classic_skullAI : public GameObjectAI
    {
        go_heroic_classic_skullAI(GameObject* gameObject) : GameObjectAI(gameObject) { }

        bool CanBeSeen(Player const* player) override
        {
            return player && player->GetDungeonDifficulty() == DUNGEON_DIFFICULTY_HEROIC;
        }
    };

    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_heroic_classic_skullAI(go);
    }
};
